...
- **`rules/crs/`: opt-in OWASP CRS v4.9.0 paranoia-level-1 bundles**, generated by the translator above: `crs-pl1.json` (101 request rules: scanner UAs, protocol attacks, LFI/RFI, RCE, PHP/Java/Node injection, XSS, SQLi, session fixation) and `crs-pl1-response.json` (50 phase-4 rules: SQL/PHP/Java/IIS error leakage, directory listings, web-shell fingerprints). Every rule is `action: "log"` with its CRS severity as the score, so the bundle is advisory by default; `log_scores_block` with `anomaly_threshold 5` gives CRS-style anomaly blocking. `COVERAGE.md` documents the 353 CRS rules that were not ported and why (260 control-flow, 57 chained, 19 non-regex operators such as `@detectSQLi`, …), and `tuning.txt` records the three rules that lose the raw `BODY` target because they match JSON/YAML/multipart framing (921150, 932130, 942190). The bundle **complements** `rules.json` rather than replacing it: CRS catches the classic `' OR 1=1--` tautology with libinjection, which has no regex form, and `rules.json` covers it. `crs_bundle_test.go` loads the bundle in CRS mode and pins 28 benign browser-shaped requests that must pass, 24 textbook attacks that must block, per-rule regex coverage, the advisory-by-default behaviour, and that known gap. Cost: about 0.6 ms per small JSON POST for the request bundle (`BenchmarkCRSPL1BenignPOST`), growing linearly with body size; see `rules/crs/README.md`.
- **`rules/crs/crs-pl2.json`, `crs-pl2-response.json`, `crs-pl3.json`, `crs-pl4.json`**: the OWASP CRS paranoia-level 2–4 rules (54 + 1, 22 and 5 rules), cumulative on top of PL1. CRS documents these levels as needing per-site tuning, and matching a raw body adds JSON/YAML syntax to what the stricter rules see, so `TestCRSPL2BenignCorpusPinned` pins exactly which benign corpus requests PL1+PL2 blocks (six of 29: JSON bodies via `942200`, a GraphQL query via `942150`, CI YAML via `941380`, a prose field via `942200` plus the advisory `920230`) and `rules/crs/README.md` explains how to tune them out. `932236` (2–3 character Unix commands after a separator) is removed in `tuning.txt` because the raw query string puts `&id=` in front of every `id` parameter.
